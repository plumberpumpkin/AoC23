...
pub fn puzzle4(input: String) -> u32 {
    ...
    for card in cards {
        ...
        println!(
            "Card {} has {} winning numbers",
            card.card_number, number_of_winning_numbers
        );
        if number_of_winning_numbers == 0 {
            points += 0;
        } else {
            let mut points_for_card: u32 = 0;
            for _i in 0..number_of_winning_numbers {
...
fn get_cards(input: Vec<&str>) -> Vec<Card> {
    ...
    for raw_card in input {
        ...
        card.card_number = raw_card_intermediate_info[0]
            ...
            .parse::<u32>()
            .unwrap();
        let raw_card_numbers_complete: Vec<&str> =
            raw_card_intermediate_info[1].split("|").collect();
        let mut card_numbers: Vec<u32> = Vec::new();
        let mut card_winning_numbers: Vec<u32> = Vec::new();
        let raw_card_numbers: Vec<&str> = raw_card_numbers_complete[1].split_whitespace().collect();
        let raw_card_winning_numbers: Vec<&str> =
            raw_card_numbers_complete[0].split_whitespace().collect();
