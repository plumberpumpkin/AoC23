...
pub fn puzzle6(input: String) -> u128{
    ...
    for i in 0..total_races{
        let race = (raw_times[i].parse::<u128>().unwrap(), raw_distances[i].parse::<u128>().unwrap());
        races.push(race);
    }
    for race in races{
...
pub fn puzzle6advanced(input: String) ->u128 {
    let raw_input:Vec<&str> = input.split("\n").collect();
    let time: u128 = raw_input[0].replace("Time:", "").replace(" ", "").parse::<u128>().unwrap();
    let distance: u128= raw_input[1].replace("Distance:", "").replace(" ", "").parse::<u128>().unwrap();
    ...
    for i in 0..time {
        ...
        if current_distance > distance {
            println!("Found a lower bound winnable solution: {}, {}", velocity, distance);
    ...
    for i in 0..time {
        ...
        if current_distance > distance {
            println!("Found an upper bound winnable solution: {}, {}", velocity, distance);
